...
Після локального запуску `npm test` збережіть скріншот термінала як `docs/tests-passed.png` та вставте його сюди.

У поточному середовищі встановлення npm-залежностей не завершилось через обмеження часу/мережі, тому я не додаю фіктивний скріншот і не заявляю про неперевірений результат.

## Demo

Посилання на deployed demo можна додати після публікації проєкту на Vercel або Netlify.
